...
export default function LeadEditModal({ isOpen, onClose, leadId, onSuccess }: LeadEditModalProps) {
  ...
  const fetchLeadData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        isCompleted: lead.isCompleted || false,
        isCompletedDate: lead.isCompletedDate ? new Date(lead.isCompletedDate).toISOString().split('T')[0] : '',
        notes: lead.notes || '',
  ...
  const handleInputChange = (field: string, value: any) => {
    setFormData(prev => {
      ...
      if (field === 'leadStatusId') {
        const selectedStatus = dropdownData.leadStatuses.find(s => s._id === value);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (leadActivityData.isCompletedDate) {
        leadActivityData.isCompletedDate = new Date(leadActivityData.isCompletedDate).toISOString();
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        <div className="bg-green-50 p-4 rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {(() => {
              const selectedStatus = dropdownData.leadStatuses.find(s => s._id === formData.leadStatusId);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center space-x-3">
              <input
                ...
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
              <label htmlFor="isCompleted" className="text-sm font-medium text-gray-700">Mark as Completed</label>
            </div>
            {formData.isCompleted && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('isCompletedDate', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
            )}
